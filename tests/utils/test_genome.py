...
class TestAnnotatedGenomeDatasetAuxStream(unittest.TestCase):
    ...
    def test_aux_stream_load_and_align_and_window(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Build aux stream pickle with raw values
            channels = ['dg50', 'gc50']
            aux_items = []
            aux_items.append({
                'sequence_id': 'ctg1',
            ...
            # Create dataset with windowing; ensure aux aligns and pads
            ds = AnnotatedGenomeDataset(
                str(fa), str(tsv), window=20, stride=15, aux_stream_path=str(aux_pkl), random_prefix_ns=False,
    ...
    def test_aux_stream_length_mismatch_raises(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Wrong length (9 instead of 10)
            channels = ['dg50']
            aux_items = [{
                'sequence_id': 'ctg1',
                'channels': channels,
                'data': np.random.randn(9, 1).astype(np.float32),
            }]
            aux_pkl = td / 'aux.pkl'
            with open(aux_pkl, 'wb') as f:
                pickle.dump(aux_items, f)
    ...
    def test_aux_stream_prefix_padding(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Enable random prefix; ensure left padding adds zeros in aux
            ds = AnnotatedGenomeDataset(str(fa), str(tsv), aux_stream_path=str(aux_pkl), random_prefix_ns=True,
                                        allow_nonconforming_start=True, allow_nonconforming_stop=True, allow_nonconforming_ass=True, allow_nonconforming_dss=True,
                                        aux_normalize=False)
            # Find contig index 0 and inspect aux
            _, _, a = ds[0]
            # a is full contig (no windowing); pad_len>0 ⇒ leading zeros exist
            # We don't know pad length; just assert some leading zeros exist and non-zeros exist later
