...
class TestMergeSplit(unittest.TestCase):
    def test_merge_and_split_basic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Only sequences that have TSV rows should be considered valid
            self.assertEqual(set(valid_sids), {"s1", "s3"})
            ...
            self.assertTrue(os.path.exists(out_train_fna))
            self.assertTrue(os.path.exists(out_test_fna))
            self.assertTrue(os.path.exists(out_train_tsv))
            self.assertTrue(os.path.exists(out_test_tsv))

    def test_pick_splits_errors(self):
